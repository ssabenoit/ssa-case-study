...
-- Aggregates per-game data to get regular season stats for each team
...
team_stats as (
    select
        ...
        sum(pp_goals) / nullif(sum(pp_attempts), 0) as pp_pct,
        sum(pim) as pim,
        sum(pk_attempts) as pk_attempts,
        (sum(pk_attempts) - sum(pk_goals_against)) / nullif(sum(pk_attempts), 0) as pk_pct,
