...
-- Regular-season stats for each skater (one row per season/player/team stint).
-- Sourced from fct_player_game_stats so play-by-play-derived detail (real
-- assists splits, GWG/OT/SH/EN goals, true faceoff counts) rolls up for free.
...
select
    ss.*,
    -- real faceoff percentage from event counts (not an average of averages)
    ...
    (ss.games_played >= {{ var('leaderboard_min_gp_skater') }}) as is_qualified,
